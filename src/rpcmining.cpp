...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "KarpelesCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "KarpelesCoin is downloading blocks...");
...
Value getworkaux(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "KarpelesCoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "KarpelesCoin is downloading blocks...");
